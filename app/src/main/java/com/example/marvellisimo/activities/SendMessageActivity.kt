...
class SendMessageActivity : AppCompatActivity() {
    ...
    var comicFromDatabase: RealmComicEntity? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        toUser = intent.getParcelableExtra<User>(ComicDetailsActivity.USER_KEY)
...
//        toUser = intent.getParcelableExtra<User>(ComicsPageActivity.USER_KEY)
        val comicId = intent.getIntExtra(ComicDetailsActivity.COMIC_KEY, 1)
        comicFromDatabase = ComicDetailsActivity.realm.where(RealmComicEntity::class.java)
            .equalTo("id", comicId)
            .findFirst()

        val comicLink = comicFromDatabase?.urls?.get(0)?.url.toString()


        performSendMessage(comicLink)
    ...
    private fun performSendMessage(text:String) {
        val fromId = FirebaseAuth.getInstance().uid
       /* val toUser = intent.getParcelableExtra<User>(ComicDetailsActivity.USER_KEY)*/
        val toId = toUser?.uid
        Log.d(TAG,"${toUser?.username}")
        ...
        reference.setValue(chatMessage)
            .addOnSuccessListener {
                ...
                Log.d(TAG, "Saved our chat message: ${text}")
